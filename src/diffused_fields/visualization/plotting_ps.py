...
class PolyscopeVisualizer:
    def __init__(
        ...
        self.vector_radius = vector_radius
        pass

    def set_camera_and_plane(self):
        # ps.set_up_dir("z_up")
        # ps.set_front_dir("neg_y_front")
        # first is the camera position then the point that camera points to and
        # lastly the up direction o fthe camera
        ps.look_at_dir(
            1.5 * np.array((-1.0, -0.5, 0.5)),
            np.array((0, 0, 0.15)),
            up_dir=(0.0, 0.0, 1.0),
        )
    ...
    def plot_vectors_on_grid(self, pcloud, grid):
        ps_pcloud = plot_orientation_field(pcloud.vertices, name="object")
        ps_pcloud.add_vector_quantity("normals", pcloud.normals, enabled=True)

        ps_grid = plot_orientation_field(grid.vertices, name="grid", point_radius=1e-5)
        ps_grid.add_vector_quantity("normals", grid.vectors, enabled=True)

    def plot_sources(self, sources, bases=None):
        plot_orientation_field(
            sources, bases=None, name="sources", point_radius=0.02, color=[1, 0, 0]
        )

    def plot_orientations_on_pointcloud(self, pcloud):
        ps_pcloud = plot_orientation_field(
            pcloud.vertices, bases=pcloud.local_bases, name="object"
        )

    def plot_orientations_on_grid(self, pcloud, grid):

        ps_grid = plot_orientation_field(
            grid.vertices, grid.local_bases, name="grid", point_radius=1e-5
        )

    def plot_trajectory(self, trajectory, trajectory_bases):
        ps_trajectory = plot_orientation_field(
            trajectory, bases=trajectory_bases, name="trajectory"
        )
    ...
    def plot_franka_hand_trajectory(
        self, position_trajectory, orientation_trajectory, name=""
    ):
        mesh, center_offset = self.import_franka_hand()

        for i, position in enumerate(position_trajectory):
            mesh_c = copy.deepcopy(mesh.mesh)
            target_pos = position + center_offset
            mesh_c.translate(target_pos, relative=False)
            rot = (
                orientation_trajectory[i]
                @ R.from_euler("xyz", [0, 0, 0], degrees=False).as_matrix()
            )
            mesh_c.rotate(rot, center=mesh_c.get_center() - center_offset)

            vertices = np.asarray(mesh_c.vertices)
            faces = np.asarray(mesh_c.triangles)
            ps_mesh_source = ps.register_surface_mesh(f"{name} {i}", vertices, faces)
